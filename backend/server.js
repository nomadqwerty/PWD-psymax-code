...
const rtcHandler = () => {
  ...
  */
    socket.on('createRoom', async (callback) => {
      ...
      for (let i = 0; i < rooms.length; i++) {
        if (
          rooms[i].callerId === socket.id ||
          rooms[i].answerId === socket.id
        ) {
          isInRoom = true;
        }
      }
      ...
      if (sender) {
        ...
        for (let i = 0; i < rooms.length; i++) {
          if (rooms[i].callerId === socket.id) {
            ...
            rooms[i].hostProducerTransport.on(
              'dtlsstatechange',
              (dtlsState) => {
                if (dtlsState === 'closed') {
            ...
            callback({
              ...
              params2: {
                ...
                iceParameters:
                  rooms[i].hostAudioProducerTransport.iceParameters,
                iceCandidates:
                  rooms[i].hostAudioProducerTransport.iceCandidates,
                dtlsParameters:
        ...
        for (let i = 0; i < rooms.length; i++) {
          if (rooms[i].answerId === socket.id) {
            ...
            rooms[i].clientAudioConsumerTransport =
              clientAudioConsumerTransport;
    ...
    socket.on('transport-recv-connect', async ({ dtlsParameters, isAudio }) => {
      ...
      for (let i = 0; i < rooms.length; i++) {
        if (rooms[i].answerId === socket.id && isAudio !== true) {
          ...
          await rooms[i].clientAudioConsumerTransport.connect({
            dtlsParameters,
          });
        }
      }
      ...
      if (sender) {
        ...
        for (let i = 0; i < rooms.length; i++) {
          if (rooms[i].answerId === socket.id) {
            ...
            rooms[i].clientAudioProducerTransport =
              clientAudioProducerTransport;
        ...
        for (let i = 0; i < rooms.length; i++) {
          if (rooms[i].callerId === socket.id) {
            ...
            rooms[i].hostConsumerTransport.on(
              'dtlsstatechange',
              (dtlsState) => {
                if (dtlsState === 'closed') {
            ...
            callback({
              ...
              params2: {
                ...
                iceParameters:
                  rooms[i].hostAudioConsumerTransport.iceParameters,
                iceCandidates:
                  rooms[i].hostAudioConsumerTransport.iceCandidates,
                dtlsParameters:
        ...
        for (let i = 0; i < rooms.length; i++) {
          if (rooms[i].answerId === socket.id) {
            if (msRoom === false && isAudio !== true) {
              ...
              await rooms[i].clientProducerTransport.connect({
                dtlsParameters,
              });
        ...
        for (let i = 0; i < rooms.length; i++) {
          ...
          if (rooms[i].answerId === socket.id) {
            if (msRoom === false && isAudio === true) {
              console.log(msRoom);
              console.log(kind);
              let producer = await rooms[
                i
              ].clientAudioProducerTransport.produce({
                kind,
                rtpParameters,
              });
    ...
    socket.on(
      ...
      async ({ dtlsParameters, isAudio }) => {
        ...
        for (let i = 0; i < rooms.length; i++) {
          ...
          if (rooms[i].callerId === socket.id && isAudio === true) {
            await rooms[i].hostAudioConsumerTransport.connect({
              dtlsParameters,
            });
          }
        }
