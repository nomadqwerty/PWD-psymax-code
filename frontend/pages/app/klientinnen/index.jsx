...
import { PAGINATION_LIMIT, SOMETHING_WRONG } from '../../../utils/constants';
  ...
  async function fetchActiveKlient(pagenum) {
    try {
      const getActive = await axiosInstance.get(
        `/klient/getActive?page=${pagenum}&pageSize=${PAGINATION_LIMIT}`
      );
  ...
  async function fetchArchivedKlient(pagenum) {
    try {
      const getArchived = await axiosInstance.get(
        `/klient/getArchived?page=${pagenum}&pageSize=${PAGINATION_LIMIT}`
      );
  ...
  async function fetchNewKlient(pagenum) {
    try {
      const getNew = await axiosInstance.get(
        `/klient/getNew?page=${pagenum}&pageSize=${PAGINATION_LIMIT}`
      );
  ...
  const BehandlungData = () => {
    return (
      <>
        ...
        <div
          className="flex items-center mt-2 cursor-pointer"
          onClick={() => router.push('/app/questionnaire/assignment')}
        >
          <svg
            className="mr-2"
            width="32"
        ...
        <div
          className="flex items-center mt-2 cursor-pointer"
          onClick={() => router.push('/app/questionnaire/evaluation')}
        >
          <svg
            className="mr-2"
            width="32"
