...
const FullRtc = () => {
  ...
  const Message = ({ message, localClientName }) => {
    return (
      <li
        key={message.id}
        className={`msgItem mb-1 ${message.clientName === localClientName ? 'right' : 'left'}`}
      >
        <p
          className={`m-0 mb-1 clientNameDate ${message.clientName === localClientName ? 'right' : 'left'}`}
        >
          <span className="clientName"> {message.clientName} </span>
          <span className="chatTimeStamp">{message.time} </span>
        </p>
        <p
          className={`msg m-0 ${message.clientName === localClientName ? 'right' : 'left'}`}
        >
          {message.message}
        </p>
      </li>
        ...
        <Col xs={12} className="footer-container">
          <Row className="footer p-3" style={{ backgroundColor: 'white' }}>
            ...
            <Col id="controls" className="">
              ...
              {/* TODO: send client back to dashboard instead of lobby */}

              <a
                href="/dashboard/videosprechstunde/lobby"
                title="Anruf beenden"
              >
